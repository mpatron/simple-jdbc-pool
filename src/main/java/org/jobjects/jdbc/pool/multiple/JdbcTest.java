...
public class JdbcTest {
	public static void main(String args[]) throws Exception {
		 ...
		 * stmt.executeQuery("select sysdate from dual"); JdbcInfo.Affiche(rs);
		 * rs.close(); stmt.close();
		 */
		// JdbcInfo.Affiche(connection.getMetaData().getImportedKeys("",
		// "CATALOG", "PRODUCT_DATES"));
		JdbcInfo jdbcinfo = new JdbcInfo(connection, "CATALOG");
		String tablename = "PRODUCTS";
		System.out.println(connection.getMetaData().getProcedureTerm());

		Iterator<JdbcField> it = jdbcinfo.getColumns(tablename.toUpperCase())
				.iterator();
		while (it.hasNext()) {
		...
		// Enumeration e =
		// jdbcinfo.getColumns(tablename.toUpperCase()).elements();
